...
object StreamsAndTopic extends IOApp.Simple {
  ...
  val program = for {
    ...
    consumer1 <- topic.subscribe(10).evalMap { str => IO.println(s"Consumer[A] consumed $str") }.compile.drain.start
    consumer2 <- topic.subscribe(10).evalMap { str => IO.println(s"Consumer[B] consumed $str") }.compile.drain.start
    writerStream = topic.publish.compose[(FiniteDuration, String)] { case (time, string) => fs2.Stream.awakeEvery[IO](time).map(_ => string) }
